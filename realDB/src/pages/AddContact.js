...
const AddContact = () => {
  ...
  const imagePicker = async e => {
    ...
    try{
      ...
      const storageRef = await firebase.storage().ref()//get storage loaction in firebase

      const uploadTask = storageRef
      .child('images/' + file.name)
      .put(resizeImage, metadata)//create location or if exists add the image with metadata

      //event while uploading
      uploadTask.on(
        firebase.storage.TaskEvent.STATE_CHANGED,//when ever we have uploaded downloaded or any change is done
        snapshot => {
          //show progress bar when ever any change is happened
          setIsUploading(true)
          var progress = (snapshot.bytesTransferred/snapshot.totalBytes) * 100

          //handle uploading event in firebase
          ...
          //If progress meets the given limit then it is either blocked or struck
          if(progress === 100){
            setIsUploading(false)
            toast("uploaded", { 
              type: "success" 
        ...
        //if everything goes right we get a callbackwhere in we get the URL to download
        () => {
  ...
  const declareDate = () => {
    let date = new Date()
    // date = "Date: "+ date.getDate()+"/"+date.getMonth()+"/"+date.getUTCFullYear()+", Time:"+date.getHours()+":"+(date.getMinutes() < 10 ? '0' : '') + date.getMinutes()+":"+(date.getSeconds() < 10 ? '0' : '')+date.getSeconds()
